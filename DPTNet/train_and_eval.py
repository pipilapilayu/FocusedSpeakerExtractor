...
from others.data import AudioDataset, AudioDataLoader
...
parser.add_argument(
    "--train_dir",
    type=str,
    default="/data/WSJ/wsj_data/min/tr",
    help="directory including mix.json, s1.json and s2.json",
)
parser.add_argument(
    "--valid_dir",
    type=str,
    default="/data/WSJ/wsj_data/min/cv",
    help="directory including mix.json, s1.json and s2.json",
)
parser.add_argument("--sample_rate", default=8000, type=int, help="Sample rate")
parser.add_argument(
    "--segment", default=4.0, type=float, help="Segment length (seconds)"
)
parser.add_argument(
    "--cv_maxlen",
    default=8.0,
...
parser.add_argument(
    "--N", default=64, type=int, help="Dim of feature to the DPT blocks"
)
parser.add_argument(
    "--W", default=2, type=int, help="Filter length in encoder, stride is W//2"
)
parser.add_argument("--K", default=250, type=int, help="Chunk size in frames")
parser.add_argument("--D", default=6, type=int, help="Number of DPT blocks")
...
parser.add_argument("--epochs", default=100, type=int, help="Number of maximum epochs")
parser.add_argument(
    "--early_stop",
    dest="early_stop",
    default=0,
...
def main(args):
    ...
    tr_dataset = AudioDataset(
        args.train_dir, batch_size=1, sample_rate=args.sample_rate, segment=args.segment
    )
    ...
    tr_loader = AudioDataLoader(tr_dataset, batch_size=1, shuffle=args.shuffle)
